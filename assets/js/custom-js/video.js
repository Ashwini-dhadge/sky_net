...
function listVideo(data = "") {
	...
	video = $("#sub_section_tbl").DataTable({
		...
		order: [[0, "desc"]],
		ajax: {
			url: base_url + _admin + "Section/subsection_list",
			type: "POST",
			dataSrc: "data",
			data: data,
		},
...
function listVideolist(data = "") {
	...
	user = $("#video_csvtable").DataTable({
		...
		order: [[0, "desc"]],
		ajax: {
			url: base_url + _admin + "Section/listVideolist",
			type: "POST",
			data: function (d) {
				d.v_id = $("#v_id").val();
			},
...
function mcqModal(lesson_video_id = "", lesson_id = "") {
	$.ajax({
		url: base_url + _admin + "Section/csvMCQModal",
		type: "POST",
...
function videoModal(id = "") {
	$.ajax({
		url: base_url + _admin + "Section/addVideoModal",
		type: "POST",
		data: { id: id },
		dataType: "json",
		success: function (res) {

...
class User extends CI_Controller
{
    ...
    public function add($_id = '')
    {
        ...
        if ($post) {
            ...
            //   $instructor['role']=4;
            // print_r($instructor);die;
            $otpNumber = create6NumRandom();
            $selfReferral = "LMS" . $instructor['mobile_no'];
            ...
            if ($post['id'] == '') {
                ...
                if ($id = $this->CommonModel->iudAction('tbl_users', $instructor, 'insert')) {
                    if ($role == 4) {
                        ...
                        // sendMobileMessage($verificationMessage, $instructor['mobile_no'],'1507163947670092160');
                ...
                $this->CommonModel->iudAction('tbl_users', $instructor, 'update', array('id' => $post['id']));
                if ($role == 4) {
                    $this->session->set_flashdata('success', 'Instructor Updated Successfully');
                } else {
    ...
    public function checkMobile()
    {
        ...
        if ($id == 0) {
            $total_row = $this->CommonModel->getData('tbl_users', array('mobile_no' => $mobile_number, 'is_deleted' => 0), '', '', 'num_rows');
            ...
            $row = $this->CommonModel->getData('tbl_users', array('mobile_no' => $mobile_number, 'is_deleted' => 0), 'id', '', 'row_array');
            if ($row['id'] == $id) {
                $output = array(
                    'success' => true
                );

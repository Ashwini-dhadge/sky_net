...
class Section extends CI_Controller
{
	...
	// 			// ⭐ If editing, override from DB
	// 			$selected_course_id = $data['lesson'][0]['course_id'];
	// 		}

	// 		$data['selected_course_id'] = $selected_course_id;  // ⭐ SEND TO VIEW

	// 		$data['active'] = 'Lesson';
	// 		$data['title'] = isset($id) ? "Edit Section" : "Add Section";
	...
	// 				redirect(base_url(ADMIN . 'Section/view/' . $post['id']));
	// 			}
	// 		}
	...
	{
		if (!$this->input->post()) {
			// ⭐ get course_id if coming from course view
			$selected_course_id = $this->input->get('course_id');
		...
		if (!empty($post['id'])) {
			...
			$this->db->insert('tbl_section', $sectionData);
	...
	public function addSection()
	{
		...
		$section_id = $this->CommonModel->iudAction('tbl_section', $section_info, 'insert');
		if (isset($post['sub_section']) && !empty($post['sub_section'])) {
			foreach ($post['sub_section'] as $key => $sub) {
				$sub_section_info = [
					'course_id' => $post['course_id'] ?? '',
				...
				$this->CommonModel->iudAction('tbl_sub_section', $sub_section_info, 'insert');
			}
		}
		if (!empty($section_id)) {
			...
			$this->session->set_flashdata('error', 'Error! fail to add Section');
			redirect(base_url(ADMIN . '/' . SECTION));
		}
	}
	...
	public function addVideo($id)
	{
		...
		// $data['lesson'] = $this->CourseModel->getLessonData('', 0, 0, 0, 0, 0, $id);
		$this->load->view(ADMIN . LESSON . 'add-video', $data);
	}

	public function addVideoMapping()
	{
		echo "<pre>";
	...
	public function view($_id)
	{
		...
		if ($lesson) {
			...
			$this->load->view(ADMIN . 	SECTION . 'table_video', $data);
		}
	}
	...
	public function addVideoModal()
	{
		...
		$html = $this->load->view(ADMIN . SECTION . 'video-modal', $data, true);
		if ($html) {
			$response['html'] = $html;
			$response['result'] = true;
			$response['reason'] = 'Data Found';
	...
	public function getSubSections()
	{
		...
		if (isset($search) && !empty($search)) {
			$sub_sections = $this->CommonModel->getData('tbl_sub_section', array('section_id' => $section_id, 'deleted_by' => NULL, 'sub_title LIKE' => "%$search%"), 'id,sub_title AS text', '', 'result_array');
		} else {
			$sub_sections = $this->CommonModel->getData('tbl_sub_section', array('section_id' => $section_id, 'deleted_by' => NULL), 'id,sub_title AS text', '', 'result_array');
		}	// die;
		// echo "<pre>";
	...
	public function subsection_list()
	{
		...
		// }
		//print_r($o_id);die;
		$count = count($this->CourseModel->getSubSectionData($searchVal, 0, 0, 0, 0, $v_id, $where));
		//print_r($count);die;
		if ($count) {
			$result = $this->CourseModel->getSubSectionData($searchVal, $sortColIndex, $sortBy, $limit, $offset, $v_id, $where);
	...
	public function sectionTypeUpdate()
	{
		if ($this->input->post()) {
			$id = $this->input->post('id');

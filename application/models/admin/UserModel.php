...
class UserModel extends CI_Model
{
    ...
    public function getUserCourseData($searchVal = '', $sortColIndex = 0, $sortBy = 'desc', $limit = 0, $offset = 0, $id = 0, $where = '')
    {
        ...
        $this->db->select('od.*,o.order_no,c.title as course_name,c.course_type as course_type,dm.name as duration_name,dm.no_of_days,o.date as order_date,o.order_status');
        if ($id) {
            $this->db->where('u.id', $id);
        }
    ...
    public function getUserCourseSubscriptions($user_id)
    {
        $this->db->select('
        s.course_id,
        s.start_date,
        ...
        o.payment_type
    ');
        $this->db->from('tbl_order_courses_subscription s');
    ...
    public function getUserCourseSubscriptionMap($user_id)
    {
        $subscriptions = $this->getUserCourseSubscriptions($user_id);
